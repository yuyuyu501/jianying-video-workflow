...
ready for captions only after its rough-cut plan is accompanied by five
validated artifacts: `visual` (silent MP4), `narration` (M4A), `review`
(muxed MP4), `transcript` (generated from the rendered review MP4), and `qc`
(successful JSON report). `caption-template` exposes the actual rough-cut
timestamps for text review. `captions` uses only the reviewed template; it
must never map a raw-camera transcript through retained EDL ranges.
...
- `speech_timeline.json`: the same final times plus `source_id`, source video,
  rough-cut-relative source start/end, role, audio policy, and actual detected
  rough-cut speech ranges.

It also writes `captions.qc.json`. This report must have `status: succeeded`
before asset selection or draft construction. It rejects uncovered opening,
middle, or final speech from the rendered rough-cut output. When a reference
script is available, pass it to `captions --reference-script`; caption QC also
rejects a short subtitle tail that is skipped before the next subtitle begins.
